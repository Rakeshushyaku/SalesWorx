...
Partial Public Class CopyRoutePlanByWeekDays
    ...
    Private Sub CopyRoutePlanByWeekDays_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        Me.Title = "Copy Route Plan By Week Days"
    End Sub
        ...
        If (Not IsPostBack) Then
            Try
                ...
                If Not Err_Desc Is Nothing Then
                    log.Error(Err_Desc)
                    Response.Redirect("Information.aspx?mode=1&errno=" & Err_No & "&msg=" & AppMsgHandler.GetErrorMessage("E_BO_RoutePlanner_005") & "&next=Welcome.aspx&Title=Copy+Route+Plan+By+Week+Days")
                End If
            Catch ex As Exception
                ...
                End If
                Response.Redirect("Information.aspx?mode=1&errno=" & "74076" & "&msg=" & AppMsgHandler.GetErrorMessage("E_BO_RoutePlanner_005") & "&next=Welcome.aspx&Title=Copy+Route+Plan+By+Week+Days")
            Finally
                objRoute = Nothing
            End Try
    ...
    Private Sub BindPlans(ByVal Id As Integer, ByVal Site As String)
        Try
            ...
            Route_ID.DataBind()
            Route_ID.Items.Insert(0, New ListItem("  -- Select a route plan -- "))
            Dim i As Integer
            ...
            RP_ID.DataSource = Dt
            RP_ID.DataBind()
            RP_ID.Items.Insert(0, New ListItem("  -- Select a default plan -- "))
    ...
    Protected Sub CopyBtn_Click(ByVal sender As Object, ByVal e As EventArgs) Handles CopyBtn.Click
        objRoute = New RoutePlan
        'If Route_ID.SelectedIndex > 0 And RP_ID.SelectedIndex > 0 Then
        '    If RP_ID.SelectedValue.ToString() = Route_ID.SelectedValue.ToString() Then
        '        Response.Redirect("Information.aspx?mode=0&msg=From and to +Route+plan+should not be+same&next=CopyRoutePlanByWeekDays.aspx&Title=Copy+Route+Plan+By+Week+Days", False)
        '        Exit Sub
        '    End If
        'End If
        Try
            ...
            'VID = s1(1)
            ' End If
            If objRoute.CopyFSRPlanByWeekDays(Err_No, Err_Desc, RP_ID.SelectedItem.Value) Then
                objLogin.SaveUserLog(Err_No, Err_Desc, "I", "CALL PLANNING", "COPY ROUTE PLAN BY WEEK DAYS", VID.Trim(), "Copy From: " & Route_ID.SelectedItem.Text & "/ Copy To :  " & RP_ID.SelectedItem.Text, CType(Session("User_Access"), UserAccess).UserID.ToString(), VID.Trim(), "0")
                Response.Redirect("Information.aspx?mode=0&msg=FSR+Route+plan+copied+successfully&next=CopyRoutePlanByWeekDays.aspx&Title=Copy+Route+Plan+By+Week+Days", False)
                Session.Remove("FSRTable")
            Else
                log.Error(Err_Desc)
                Response.Redirect("Information.aspx?mode=1&errno=" & Err_No & "&msg=" & AppMsgHandler.GetErrorMessage("E_BO_RoutePlanner_005") & "&next=CopyRoutePlanByWeekDays.aspx&Title=Copy+Route+Plan+By+Week+Days", False)
            End If
        Catch ex As Exception
            Err_No = "74074"
            Err_Desc = ex.Message()
    ...
    Protected Sub Route_FSR_ID_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles Route_FSR_ID.SelectedIndexChanged
        ...
        Catch ex As Exception
            ...
            Else
                log.Error(GetExceptionInfo(ex))
            End If
            Response.Redirect("Information.aspx?mode=1&errno=" & "74075" & "&msg=" & AppMsgHandler.GetErrorMessage("E_BO_RoutePlanner_005") & "&next=CopyRoutePlanByWeekDays.aspx&Title=Copy+Route+Plan+By+Week+Days", False)
        End Try
