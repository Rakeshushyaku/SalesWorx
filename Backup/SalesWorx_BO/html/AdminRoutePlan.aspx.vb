...
Partial Public Class AdminRoutePlan
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
        If (Action_Mode.Value = "MODIFY" Or Action_Mode.Value = "APPROVE") Then
    ...
    Private Sub DefPlanCalendar_DayRender(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DayRenderEventArgs) Handles DefPlanCalendar.DayRender
        Try
            ...
            End If
            If (Action_Mode.Value = "APPROVE") Then
                ' Try
                Dim SDate As String = Start_Date.Text.Trim()
                Dim EDate As String = End_Date.Text.Trim()
                Dim CellText As String
    ...
    Private Sub SaveBtn_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles SaveBtn.Click
        ...
        Try
            ...
            If objRoutePlan.CheckFSRPlanID(Session("Default_Plan_ID"), Session("SalesRep_ID"), Err_No, Err_Desc) = True Then
                Response.Redirect("information.aspx?mode=0&msg=Route+plan+already+created&next=CreateRoutePlan.aspx&Title=Route+Planner", False)
                Exit Sub
            End If
            ...
            VCount = 0
            Response.Redirect("information.aspx?mode=0&msg=Route+plan+saved+successfully&next=CreateRoutePlan.aspx&Title=Route+Planner", False)
        Catch ex As Exception
            Err_No = "74042"
            Err_Desc = ex.Message
            If (Err_Desc Is Nothing) Then
                        ...
                        If Not (SubStrArr(0).ToString() = "") Then
                            ...
                            Else
                                If DivName.Length > 4 Then
                                    StrBuilder.Append(vbCrLf & "<br/>" & CountandBreak(DivName(1), HasTimeVal))
                                Else
                                    StrBuilder.Append(vbCrLf & "<br/>" & CountandBreak(DivName(1), HasTimeVal))
                                End If
                                HasTimeVal = False
    ...
    End Sub
    Protected Sub SetPanelVisibility(ByVal AddPnl As Boolean, ByVal ModPnl As Boolean, ByVal AppPanel As Boolean)
        ...
        Try
            ...
            If (IsApproved.Value = "Y") Then
                ...
                If Not (objRoutePlan.UpdateFSRPlan(Err_No, Err_Desc, SqlConn, SqlCmd, transaction)) Then
                    ...
                    log.Error(Err_Desc)
                    Response.Redirect("information.aspx?mode=1&errno=" & Err_No & "&msg=" & AppMsgHandler.GetErrorMessage("E_BO_RoutePlanner_004") & "&next=ModDelRoutePlan.aspx", False)
                    Exit Try
                End If
            End If
            ...
            Else
                ...
                Response.Redirect("information.aspx?mode=1&errno=" & Err_No & "&msg=" & AppMsgHandler.GetErrorMessage("E_BO_RoutePlanner_004") & "&next=ModDelRoutePlan.aspx&Title=Route+Planner", False)
            End If
            objRoutePlan.CloseConnection(SqlConn)
            ...
            VCount = 0
            Response.Redirect("information.aspx?mode=0&msg=Route+plan+updated+successfully&next=ModDelRoutePlan.aspx?ret=1&Title=Route+Planner", False)
        Catch ex As Exception
            Err_No = "74042"
            If Err_Desc Is Nothing Then
                log.Error(GetExceptionInfo(ex))
            ...
            Err_Desc = ex.Message
            If (Err_Desc Is Nothing) Then
                Response.Redirect("information.aspx?mode=0&msg=Route+plan+updated+successfully&next=ModDelRoutePlan.aspx?ret=1", False)
            Else
                Response.Redirect("information.aspx?mode=1&errno=" & Err_No & "&msg=" & Err_Desc & "&next=ModDelRoutePlan.aspx", False)
            End If
        Finally
            objRoutePlan = Nothing
