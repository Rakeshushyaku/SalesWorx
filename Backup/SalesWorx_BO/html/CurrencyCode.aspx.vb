...
Regex("(^[-+]?\d+(,?\d*)*\.?\d*([Ee][-+]\d*)?$)|(^[-+]?\d?(,?\d*)*\.\d+([Ee][-+]\d*)?$)")
        ...
        Try
            ...
            For Each dr In gvCurrency.Rows
                ...
                If RowCheckBox.Checked = True Then
                    ...
                    objCurrency.CurrencyCode = dr.Cells(1).Text
            ...
            Next
            If (Success = True) Then
                lblMessage.Text = "Currency code(s) deleted successfully."
                lblMessage.ForeColor = Drawing.Color.Green
                lblinfo.Text = "Information"
                MpInfoError.Show()
                btnClose.Focus()
    ...
    Protected Sub btnEdit_Click(ByVal sender As Object, ByVal e As EventArgs)
        Try
            ...
            HidVal.Value = row.Cells(1).Text
            txtCurrencyCode.Text = Trim(row.Cells(1).Text)
            txtDescription.Text = Trim(row.Cells(2).Text)
            txtRate.Text = Trim(row.Cells(3).Text)
            ddlDigits.Text = Trim(row.Cells(4).Text)
