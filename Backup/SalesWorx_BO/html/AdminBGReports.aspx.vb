 ...
 Dim Err_No As Long
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If ddl_year.SelectedItem.Value = "-1" Then
            MessageBoxValidation("Please select the Year")
            Exit Sub
        End If
        If ddl_Org.SelectedItem.Value = "-1" Then
            MessageBoxValidation("Please select the Division")
            Exit Sub
        End If
          ...
          If objBGReport.SaveBGReport(Err_No, Err_Desc, 1, "0", CType(Session("User_Access"), UserAccess).UserID, dt) Then
               BindData()
               MessageBoxInfo("Report Criteria saved sucessfully.")
              Resetfields()

           Else
             MessageBoxInfo("Error while saving. Please contact the administrator.")
           End If
...
Me.Panel.Update()
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Try
            ...
            Else
                MessageBoxValidation("Error occured while deleting.")
                log.Error(Err_Desc)
                Exit Try
            End If
  ...
  Sub BindData()
     Try
            ...
            Dim dt As New DataTable
            dt = objBGReport.GetBGReports(Err_No, Err_Desc, txt_fromDate.Text.Trim, txt_ToDate.Text.Trim, ddl_Status.SelectedItem.Value)
...
Protected Sub btnFilter_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnFilter.Click
     If txt_fromDate.Text = "" Then
         MessageBoxInfo("Enter from date")
                Exit Sub
     ElseIf Not IsDate(txt_fromDate.Text) Then
                MessageBoxInfo("Invalid from date")
                Exit Sub
     End If
     If txt_ToDate.Text = "" Then
         MessageBoxInfo("Enter to date")
                Exit Sub
     ElseIf Not IsDate(txt_ToDate.Text) Then
                MessageBoxInfo("Invalid to date")
                Exit Sub
     End If
     If IsDate(txt_fromDate.Text) > IsDate(txt_ToDate.Text) Then
                MessageBoxInfo("From date greater than to date")
                Exit Sub
     End If
    BindData()
    Me.Panel.Update()
