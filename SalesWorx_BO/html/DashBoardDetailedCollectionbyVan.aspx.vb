...
Imports log4net
Public Class DashBoardDetailedCollectionbyVan
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Try
                ...
                dt = (New SalesWorx.BO.Common.Reports).GetCollectionbyVanForMonth(Err_No, Err_Desc, HUID.Value, CDate(txtFromDate.SelectedDate).ToString("MM-yyyy"))
                ...
                If CType(Session.Item("CONTROL_PARAMS"), ControlParams).CLIENT_CODE = "GYMA" Then
                    gvRep.Columns(5).Visible = True
                    hfCC.Value = "True"
                Else
                    hfCC.Value = "False"
                    gvRep.Columns(5).Visible = False
                End If
                ...
                Dim sumCash = dt.AsEnumerable().Sum((Function(x) x.Field(Of Decimal)("CashAmount")))

                StrSummary = StrSummary & "<div class='col-sm-4'><div class='widgetblk'>Total Cash " & HCurrency.Value & "<div class='text-primary'>" & Format(sumCash, hfDecimal.Value) & "</div></div></div>"

                Dim sumPdc = dt.AsEnumerable().Sum((Function(x) x.Field(Of Decimal)("ChequeAmount")))

                StrSummary = StrSummary & "<div class='col-sm-4'><div class='widgetblk'>Total CDC " & HCurrency.Value & "<div class='text-primary'>" & Format(sumPdc, hfDecimal.Value) & "</div></div></div>"

                Dim PCDAmount = dt.AsEnumerable().Sum((Function(x) x.Field(Of Decimal)("PCDAmount")))

                StrSummary = StrSummary & "<div class='col-sm-4'><div class='widgetblk'>Total PDC " & HCurrency.Value & "<div class='text-primary'>" & Format(PCDAmount, hfDecimal.Value) & "</div></div></div>"

                If CType(Session.Item("CONTROL_PARAMS"), ControlParams).CLIENT_CODE = "GYMA" Then
                    Dim CCAmount = dt.AsEnumerable().Sum((Function(x) x.Field(Of Decimal)("CCAmount")))
    ...
    Private Sub gvRep_PreRender(sender As Object, e As EventArgs) Handles gvRep.PreRender
        For Each column As GridColumn In gvRep.MasterTableView.Columns
            If column.UniqueName = "CashAmount" Then
            ...
            End If
            If column.UniqueName = "ChequeAmount" Then
                Dim col As GridBoundColumn
                col = (CType(column, GridBoundColumn))
                col.DataType = System.Type.GetType("System.Double")
                col.DataFormatString = "{0:" & hfDecimal.Value & "}"
            ...
            End If
            If column.UniqueName = "PCDAmount" Then
                Dim col As GridBoundColumn
                col = (CType(column, GridBoundColumn))
                col.DataType = System.Type.GetType("System.Double")
                col.DataFormatString = "{0:" & hfDecimal.Value & "}"
            ...
            End If
            If column.UniqueName = "CCAmount" Then
                Dim col As GridBoundColumn
                col = (CType(column, GridBoundColumn))
                col.DataType = System.Type.GetType("System.Double")
                col.DataFormatString = "{0:" & hfDecimal.Value & "}"
