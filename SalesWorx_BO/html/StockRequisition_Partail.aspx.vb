...
Public Class StockRequisition_Partail
    ...
    Sub LoadStockSheet()
        Try
            Dim strselect = "<select class='uom' id='$UOMID' name='$UOMID'>$ITEMS$</select>"
            Dim strhiden = "<input Type='hidden' id='$hqty' name='$hqty' value='$UOMQTY'/>"
            ...
            If ddl_org.SelectedItem.Value <> "0" And ddl_Agency.SelectedItem.Value <> "0" Then
                ...
                Else
                    ...
                    dt = Stock.GetStockRequisition_UOM(Err_No, Err_Desc, ddl_Agency.SelectedItem.Value, ddl_org.SelectedItem.Value)
                    If dt.Rows.Count > 0 Then
                        Str = "<div id='main'><div style='color:red;'>Please click the Product/Sales Man to edit the quantities</div><table cellpadding='0' cellspacing='0' border='0' class='overflow-y'><thead ><tr><th>Product</th><th class='setcolor'>Total</th><th class='setcolor'>UOM</th>"
                        ...
                        For i As Integer = 0 To dt.Rows.Count - 1
                            ...
                            t_strselect = strselect
                            Dim t_itemstr As String = ""
                            Dim uomdt As New DataTable
                            uomdt = Stock.GetItemUOMs(Err_No, Err_Desc, ddl_org.SelectedItem.Value, dt.Rows(i)(1))
                            Dim Conversionfact As String = "1"
                            For Each uomdr As DataRow In uomdt.Rows
                                Dim v As String
                                v = uomdr("Item_UOM").ToString & "$" & uomdr("Conversion").ToString
                                If uomdr("Item_UOM").ToString.ToLower() = uomdr("DUOM").ToString.ToLower() Then
                                    t_itemstr = t_itemstr & "<option value='" & v & "' selected>" & uomdr("Item_UOM").ToString & "</option>"
                                    Conversionfact = uomdr("Conversion")
                                Else
                                    t_itemstr = t_itemstr & "<option value='" & v & "'>" & uomdr("Item_UOM").ToString & "</option>"
                                End If

                            Next
                            t_strselect = t_strselect.Replace("$ITEMS$", t_itemstr)
                            t_strselect = t_strselect.Replace("$UOMID", "drpUOM_" & i)
                            uomdt = Nothing
                            ...
                            Str = Str & "<td class='RUOM'" & i & "'>" & t_strselect & "</td>"

                            For j As Integer = 2 To dt.Columns.Count - 3
                                Rtoal = Rtoal + Val(dt.Rows(i)(j).ToString) / Val(Conversionfact)
                                Dim tstrhiden = strhiden
                                tstrhiden = tstrhiden.Replace("$hqty", "HUOMQty_" & i & "_" & j - 2)
                                tstrhiden = tstrhiden.Replace("$UOMQTY", Format(Val(dt.Rows(i)(j).ToString), "###0.00"))
                                Str = Str & "<td class='Qty' id='Qty_" & i & "_" & j - 2 & "'>" & tstrhiden & Format(Val(dt.Rows(i)(j).ToString) / Val(Conversionfact), "###0.00") & "</td>"
                            Next
                            Str = Str.Replace("$RTOTAL$", Format(Rtoal, "###0.00"))

                            Str = Str & "</tr>"
                        Next
                        Str = Str & "<tr class='TotalQty'><th>Total in Base UOM</th>"
                        ...
                        For i As Integer = 2 To dt.Columns.Count - 3
                            ...
                            Str = Str & "<td class='CTotal' id='CTotal_" & i - 2 & "'>" & Format(Val(sumObject.ToString()), "###0.00") & "</td>"
                            Total = Total + Val(sumObject.ToString())
                        Next

                        Str = Str.Replace("$TOTAL$", Format(Total, "###0.00"))
                        Str = Str & "</tr>"
    ...
    Sub LoadStockSheetFormOrg()
        Try
            ...
            If ddl_org.SelectedItem.Value <> "0" Then
                ...
                dt = Stock.GetStockRequisition_UOM(Err_No, Err_Desc, ddl_Agency.SelectedItem.Value, ddl_org.SelectedItem.Value)
                If dt.Rows.Count > 0 Then
                    Str = "<div id='main'><div style='color:red;'>Please click the Product/Sales Man to edit the quantities</div><table cellpadding='0' cellspacing='0' border='0' class='overflow-y'><thead ><tr><th>Product</th><th class='setcolor'>Total</th>"
                    For j As Integer = 2 To dt.Columns.Count - 3
                        Str = Str & "<th class='SM' id='SM_" & j - 2 & "'>" & dt.Columns(j).ColumnName & "<input Type='hidden' id=" & "'HSM_" & j - 2 & "' name=" & "'HSM_" & j - 2 & "' value='" & dt.Columns(j).ColumnName & "'/></th>"
                    Next
                    Str = Str & "</tr></thead><tbody>"
    ...
    Public Shared Function SaveStock(ByVal Ostock As List(Of Stock)) As String
        ...
        For Each stk As SalesWorx.BO.Common.Stock In Ostock
            Dim conv As String
            conv = Stock.GetConverion(Err_No, Err_Desc, stk.Org_ID, stk.Product, stk.UOM)
            Stock.Product = stk.Product
            Stock.SalesMan = stk.SalesMan
            Stock.Qty = Val(stk.Qty) * Val(conv)
