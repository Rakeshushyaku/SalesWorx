...
Partial Public Class Rep_SalesbyVan
    ...
    Private ReportPath As String = "SalesbyVan_V2"

    Private Const PageID As String = "P111"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Try
                ...
                txtFromMonth.SelectedDate = DateAdd(DateInterval.Month, -3, Now)
    ...
    Private Sub SearchBtn_Click(sender As Object, e As EventArgs) Handles SearchBtn.Click
        If ValidateInput() Then
            ...
            lbl_Type.Text = ddl_Type.SelectedItem.Text
            ...
            ElseIf ddl_Type.SelectedItem.Value = "1" Then
            ...
            ElseIf ddl_Type.SelectedItem.Value = "2" Then
                fromdate = DateAdd(DateInterval.Day, -1 * (CDate(txtFromMonth.SelectedDate).Day - 1), CDate(txtFromMonth.SelectedDate))
                Todate = DateAdd(DateInterval.Day, -1 * (CDate(txtFromMonth.SelectedDate).Day - 1), CDate(txtFromMonth.SelectedDate))
                Todate = DateAdd(DateInterval.Day, -1, DateAdd(DateInterval.Month, 1, Todate))
            End If
            ...
            If ddl_Type.SelectedItem.Value = "0" Then
                ...
                lbl_fromtxt.Text = "From Date:"

                HToDate.Value = Todate.ToString("dd-MMM-yyyy")
                lbl_Todt.Text = CDate(txtToDate.SelectedDate).ToString("dd-MMM-yyyy")
                lbl_Totxt.Text = "To Date:"

                BindOverAllReport()
                BindOverAllChart()
                Salestab.Tabs(0).Selected = True
                Salestab.Tabs(2).Visible = False
                Salestab.Tabs(0).Visible = True
                Salestab.Tabs(1).Visible = True
            Else
                If ddl_Type.SelectedItem.Value = "1" Then
                    HDate.Value = fromdate.ToString("dd-MMM-yyyy")
                    ...
                    Salestab.Tabs(2).Text = "Monthly Sales"
                Else
                    HDate.Value = fromdate.ToString("dd-MMM-yyyy")
                    lbl_Fromdt.Text = CDate(txtFromMonth.SelectedDate).ToString("MMM-yyyy")
                    lbl_fromtxt.Text = "Month:"

                    HToDate.Value = Todate.ToString("dd-MMM-yyyy")
                    lbl_Todt.Text = CDate(txtToMonth.SelectedDate).ToString("MMM-yyyy")
                    lbl_Totxt.Text = ""
                    lbl_Todt.Visible = False
                    lbl_Totxt.Visible = False
                    Salestab.Tabs(2).Text = "Weekly Sales"
                End If
                Salestab.Tabs(2).Visible = True
                Salestab.Tabs(0).Visible = True
                Salestab.Tabs(1).Visible = True
                BindOverAllReport()
                BindOverAllChart()
    ...
    Private Sub ddl_Type_SelectedIndexChanged(sender As Object, e As RadComboBoxSelectedIndexChangedEventArgs) Handles ddl_Type.SelectedIndexChanged
        ...
        ElseIf ddl_Type.SelectedItem.Value = "1" Then
            lbl_from.Text = "From Month"
            lbl_to.Text = "To Month"
        ...
        ElseIf ddl_Type.SelectedItem.Value = "2" Then
            ...
            txtFromMonth.Visible = True
            txtToMonth.Visible = False
            txtFromMonth.SelectedDate = Now.Date
        End If
        rptsect.Visible = False
        Args.Visible = False
    End Sub
    Sub BindOverAllReport()
        Try
            ...
            lblDesc.Text = "Sales and Returns from " & Fromdate.ToString("dd-MMM-yyyy") & " To " & Todate.ToString("dd-MMM-yyyy")
            lblDesc1.Text = "Net Sales from " & Fromdate.ToString("dd-MMM-yyyy") & " To " & Todate.ToString("dd-MMM-yyyy")

            dt = objRep.GetSalesReturnsbyVan(Err_No, Err_Desc, HorgID.Value, HVan.Value, HUID.Value, HType.Value, Fromdate.ToString("dd-MMM-yyyy"), Todate.ToString("dd-MMM-yyyy"), HAgency.Value, HItem.Value, Hcustomer.Value)
            ...
            '    Chartwrapper.Style.Add("height", (dt.Rows.Count * 45).ToString & "px")
            '    Chartwrapper1.Style.Add("height", (dt.Rows.Count * 45).ToString & "px")
            'ElseIf dt.Rows.Count > 14 Then
            '    Chartwrapper.Style.Add("height", (dt.Rows.Count * 40).ToString & "px")
            '    Chartwrapper1.Style.Add("height", (dt.Rows.Count * 40).ToString & "px")
            'Else
            '    Chartwrapper.Style.Add("height", "400px")
            '    Chartwrapper1.Style.Add("height", "400px")
            'End If
            If dt.Rows.Count > 8 And dt.Rows.Count < 14 Then
    ...
    Sub BindReport()
        ...
        dt = objRep.GetSalesReturnsbyVanDetailed(Err_No, Err_Desc, HorgID.Value, HVan.Value, HUID.Value, HType.Value, Fromdate.ToString("dd-MMM-yyyy"), Todate.ToString("dd-MMM-yyyy"), HAgency.Value, HItem.Value, Hcustomer.Value)
        ...
        dtFinal.Columns.Add("Net", Type.GetType("System.Double"))
        ...
        If dt.Rows.Count Then
            For Each dr As DataRow In dt.Rows
                ...
                drfinal("Sales") = dr("Sales")
                drfinal("Returns") = dr("Returns")
                drfinal("Net") = dr("Net")
                If ddl_Type.SelectedItem.Value = "1" Then
                    drfinal("Year") = CDate(dr("M").ToString & "/01/" & dr("Yr")).ToString("MMM-yyyy")
                Else
                    drfinal("Year") = "Week" & dr("M").ToString
                End If
                dtFinal.Rows.Add(drfinal)
            Next
        End If

        RadPivotGrid1.DataSource = dtFinal
        RadPivotGrid1.DataBind()

        lbl_Currency1.Text = HCurrency.Value
        divCurrency1.Visible = True
    ...
    Private Sub RadPivotGrid1_CellDataBound(sender As Object, e As PivotGridCellDataBoundEventArgs) Handles RadPivotGrid1.CellDataBound

        If TypeOf e.Cell Is PivotGridColumnHeaderCell Then
            If e.Cell.Text.IndexOf("Total Sum of Net") >= 0 Then
                e.Cell.Text = "Net"
            End If
                ...
                Select Case TryCast(cell.Field, PivotGridAggregateField).DataField
                    Case "Sales"
                        If cell.DataItem.ToString().Length > 0 Then
                            cell.Text = Format(CDbl(cell.DataItem), lblDecimal.Text)
                        End If
                    Case "Returns"
                        If cell.DataItem.ToString().Length > 0 Then
                            cell.Text = Format(CDbl(cell.DataItem), lblDecimal.Text)
    ...
    Private Sub RadPivotGrid1_PageIndexChanged(sender As Object, e As PivotGridPageChangedEventArgs) Handles RadPivotGrid1.PageIndexChanged
        BindReport()
    End Sub
    Private Sub Btn_LoadItem_Click(sender As Object, e As EventArgs) Handles Btn_LoadItem.Click
    ...
    Private Sub ClearBtn_Click(sender As Object, e As EventArgs) Handles ClearBtn.Click
        ...
        txtFromMonth.SelectedDate = DateAdd(DateInterval.Month, -2, Now)
        txtToMonth.SelectedDate = Now()
    End Sub
    Protected Sub gvRep_ItemCommand(sender As Object, e As GridCommandEventArgs)
        ...
        If e.CommandName = RadGrid.ExportToExcelCommandName Then
            ...
            gvRep.ExportSettings.FileName = "SalesbyVan"
        End If
    ...
    Private Sub BtnExportBiffExcel_Click(sender As Object, e As EventArgs) Handles BtnExportBiffExcel.Click
        Try
            ...
            tblData = dt.DefaultView.ToTable(False, "SalesRep_name", "Sales", "Returns", "Net")

            tblData.Columns("SalesRep_name").ColumnName = "Van"
            ...
            If tblData.Rows.Count > 0 Then
                ...
                Using package As New ExcelPackage()
                    ...
                    Response.AddHeader("content-disposition", "attachment;filename= SalesbyVan.xlsx")
