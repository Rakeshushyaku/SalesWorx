...
Public Class Rep_DelayedTrendByDiv
    ...
    Private ReportPath As String = "DelayedCollectionTrendByDiv"

    Private Const PageID As String = "P344"
    ...
    Sub Export(format As String)
        ...
        If format = "PDF" Then
            ...
            Response.AddHeader("Content-disposition", "attachment;filename=CollectionTrendByDiv.pdf")
            Response.AddHeader("Content-Length", bytes.Length)
        ElseIf format = "Excel" Then
            Response.ContentType = "application/excel"
            Response.AddHeader("Content-disposition", "filename=CollectionTrendByDiv.xls")
            Response.AddHeader("Content-Length", bytes.Length)
        End If
    ...
    Sub BindReport()
        ...
        dt = (New SalesWorx.BO.Common.Reports).GetDelayedCollectionByDiv(Err_No, Err_Desc, hfOrg.Value, CDate(txtFromDate.SelectedDate).ToString("MMM-yyyy"), CDate(txtToDate.SelectedDate).ToString("MMM-yyyy"), CType(Session("User_Access"), UserAccess).UserID, hfLoc.Value)
        If dt.Rows.Count Then
            ...
            While tfromdate <= todate
                trHdstr = trHdstr & "<th colspan='3'>" & tfromdate.ToString("MMM-yyyy") & "</th>"
            ...
            While tfromdate <= todate
                trHdstr = trHdstr & "<th>Delayed</td>"
                trHdstr = trHdstr & "<th>Paid On Time</td>"
                trHdstr = trHdstr & "<th>Delayed %</td>"
                tfromdate = DateAdd(DateInterval.Month, 1, tfromdate)
            End While
            trHdstr = trHdstr & "</tr>"
