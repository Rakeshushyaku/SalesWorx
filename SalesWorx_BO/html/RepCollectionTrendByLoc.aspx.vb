...
Public Class RepCollectionTrendByLoc
    ...
    Private ReportPath As String = "DelayedCollectionByLoc"

    Private Const PageID As String = "P329"
    ...
    Sub Export(format As String)
        ...
        If CDate(txtToDate.SelectedDate).Day = 1 Then
            todate = DateAdd(DateInterval.Day, -1, DateAdd(DateInterval.Month, 1, CDate(txtToDate.SelectedDate))).ToString("dd-MMM-yyyy") & " 23:59:59"
        Else
            todate = DateAdd(DateInterval.Day, -1, DateAdd(DateInterval.Month, 1, DateAdd(DateInterval.Day, -1 * (Now.Day - 1), CDate(txtToDate.SelectedDate)))).ToString("dd-MMM-yyyy") & " 23:59:59"
        End If
        ...
        For Each item As RadComboBoxItem In ddlLocation.CheckedItems
            LocationStr = LocationStr & item.Value & ","
        Next
        ...
        Loc = New ReportParameter("Loc", LocationStr)
        ...
        If format = "PDF" Then
            ...
            Response.AddHeader("Content-disposition", "attachment;filename=CollectionTrendByLoc.pdf")
            Response.AddHeader("Content-Length", bytes.Length)
        ElseIf format = "Excel" Then
            Response.ContentType = "application/excel"
            Response.AddHeader("Content-disposition", "filename=CollectionTrendByLoc.xls")
            Response.AddHeader("Content-Length", bytes.Length)
        End If
    ...
    Sub BindReport()
        ...
        For Each item As RadComboBoxItem In ddlLocation.CheckedItems
            LocationStr = LocationStr & item.Value & ","
            Locationtxt = Locationtxt & item.Text & ","
        Next
        ...
        HLoc.Value = LocationStr
        Dim tblstr As String = ""
        Dim dt As New DataTable
        dt = (New SalesWorx.BO.Common.Reports).GetDelayedCollectionByLocation(Err_No, Err_Desc, hfOrg.Value, CDate(txtFromDate.SelectedDate).ToString("MMM-yyyy"), CDate(txtToDate.SelectedDate).ToString("MMM-yyyy"), CType(Session("User_Access"), UserAccess).UserID, HLoc.Value)
        Dim dtFinal As New DataTable
        dtFinal.Columns.Add("Area")
        dtFinal.Columns.Add("Year")
        dtFinal.Columns.Add("Paid On Time", Type.GetType("System.Double"))
        dtFinal.Columns.Add("Delayed", Type.GetType("System.Double"))
        dtFinal.Columns.Add("Delayed Percentage", Type.GetType("System.Double"))

        If dt.Rows.Count Then
            For Each dr As DataRow In dt.Rows
                Dim drfinal As DataRow
                drfinal = dtFinal.NewRow
                drfinal("Area") = dr("Description")
                drfinal("Paid On Time") = dr("PaidOnTime")
                drfinal("Delayed") = dr("Delayed")
                drfinal("Delayed Percentage") = dr("Perc")
    ...
    Sub LoadOrgs()
        ...
        Me.lblDecimal.Text = "N" & DecimalDigits
