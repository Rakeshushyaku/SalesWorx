<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="StockRequisition_Partail.aspx.vb" Inherits="SalesWorx_BO.StockRequisition_Partail"  MasterPageFile="~/html/Site.Master" %>
...
<script type="text/javascript">
    ...
    <style>
        td.RUOM select{width:60px !important;}
    </style>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server" >
    ...
    <script language="javascript" type="text/javascript">
            ...
            require(["dojo", "dojo/dom-style", "dojo/dom-construct", "dojo/dom", "dojo/on", "dojo/query", "dojo/_base/lang", "dojo/request", "dojo/domReady!"], function (dojo, domstyle, domConstruct, dom, on, query, lang, request) {
                    ...
                    onClick: function (evt) {
                        ...
                        if (dom.byId("MainContent_HClicked").value == "0") {
                            ...
                            for (i = 0; i <= parseInt(dom.byId("MainContent_RowCount").value) ; i = i + 1) {
                                var val1
                                val1 = dom.byId("HUOMQty_" + i + "_" + indx[1]).value

                                dom.byId("drpUOM_" + i).disabled = true
                                ...
                                domConstruct.place("<input  Type='hidden' id='HUOMQty_" + i + "_" + indx[1] + "' value='" + val1 + "'></input>", cid)
                            }
                ...
                var SelectRow = {
                    ...
                    onClick: function (evt) {
                        ...
                        if (dom.byId("MainContent_HClicked").value == "0") {
                            ...
                            var i
                            dom.byId("drpUOM_" + indx[1]).disabled = true

                            for (i = colstart + 1; i < ids.length - 1; i = i + 1) {
                                var val
                                var val1
                                val1 = dom.byId("HUOMQty_" + indx[1] + "_" + (i - 3)).value
                                ...
                                domConstruct.place("<input  Type='hidden' id='HUOMQty_" + indx[1] + "_" + (i - 3) + "' value='" + val1 + "'></input>", ids[i])
                            }
                ...
                var UOMQty = {
                    ...
                    onClick: function (evt) {
                        ...
                        if (dom.byId("MainContent_HClicked").value == "0") {
                            ...
                            var conv = dom.byId(pid).value.split("$")


                            var ids = query(".Qty_" + indx[1] + "> *")
                            var i
                            var tot = 0
                            for (i = colstart + 1; i <= ids.length - colstart; i = i + 1) {

                                var val = dom.byId("HUOMQty_" + indx[1] + "_" + (i - 3)).value

                                var val1 = Math.round((val / conv[1]) * 100) / 100
                                dom.byId(ids[i]).textContent = val1
                                tot = tot + val1
                                domConstruct.place("<input  Type='hidden' id='HUOMQty_" + indx[1] + "_" + (i - 3) + "' value='" + val + "'></input>", ids[i])




                            }
                            dom.byId("RTotal_" + indx[1]).textContent = Math.round(tot * 100) / 100
                ...
                var SaveRow = {
                    ...
                    onClick: function (evt) {
                        ...
                        dom.byId("drpUOM_" + indx[1]).disabled = false


                        var uom = dom.byId("drpUOM_" + indx[1]).value.split("$")
                        ...
                        for (i = colstart + 1; i <= ids.length - colstart ; i = i + 1) {
                            ...
                            idata = idata + '{"SalesMan":"' + dom.byId("SM_" + (i - colstart - 1)).textContent + '","Product":"' + dom.byId("HPR_" + indx[1]).value + '","Qty":"' + val + '","Org_ID":"' + orgid + '","UOM":"' + uom[0] + '"},'
                        ...
                        $.ajax({
                            type: "POST",
                            url: "StockRequisition_Partail.aspx/SaveStock",
                            data: '{"Ostock":[' + idata + ']}',
                ...
                var CancelRow = {
                    ...
                    onClick: function (evt) {
                        ...
                        for (i = colstart + 1; i <= ids.length - colstart; i = i + 1) {
                            ...
                            var val1 = dom.byId("HUOMQty_" + indx[1] + "_" + (i - 3)).value
                ...
                var CancelCol = {
                    ...
                    onClick: function (evt) {
                        ...
                        for (i = 0; i <= parseInt(dom.byId("MainContent_RowCount").value) ; i = i + 1) {
                            dom.byId("drpUOM_" + i).disabled = false

                            var val = dom.byId("hqty_" + i + "_" + indx[1]).value
                            console.debug("HUOMQty_" + i + "_" + indx[1])

                            var val1 = dom.byId("HUOMQty_" + i + "_" + indx[1]).value

                            dom.byId("Qty_" + i + "_" + indx[1]).textContent = val

                            domConstruct.place("<input  Type='hidden' id='HUOMQty_" + i + "_" + indx[1] + "' value='" + val1 + "'></input>", "Qty_" + i + "_" + indx[1])
                ...
                var SaveCol = {
                    ...
                    onClick: function (evt) {
                        ...
                        for (i = 0; i <= parseInt(dom.byId("MainContent_RowCount").value) ; i = i + 1) {
                            ...
                            var uom = dom.byId("drpUOM_" + i).value.split("$")

                            var val = dom.byId("txt_" + i + "_" + indx[1]).value

                            idata = idata + '{"SalesMan":"' + dom.byId("SM_" + indx[1]).textContent + '","Product":"' + dom.byId("HPR_" + i).value + '","Qty":"' + val + '","Org_ID":"' + orgid + '","UOM":"' + uom[0] + '"},'
                        ...
                        $.ajax({
                            ...
                            dataType: "json",
                            success: function (response) {
                                //location.href = "StockRequisition_Partail.aspx?Agency=" + agency + "&OrgID=" + OrgID
                                //__doPostBack('<%=ReloadThePanel.ClientID %>', null);
                                document.getElementById("MainContent_ReloadThePanel").click()
                ...
                var SelectCell = {
                    ...
                    onClick: function (evt) {
                        ...
                        if (dom.byId("MainContent_HClicked").value == "0") {
                            ...
                            var val1
                            val1 = dom.byId("HUOMQty_" + indx[1] + "_" + i).value
                            ...
                            domConstruct.place("<input  Type='text' id='txt_" + i + "_" + indx[1] + "' value='" + val + "' onKeypress='return NumericOnly(event)'></input>", cid)
                            domConstruct.place("<input  Type='hidden' id='hqty_" + i + "_" + indx[1] + "' value='" + val + "' onKeypress='return NumericOnly()'></input>", cid)
                            domConstruct.place("<input  Type='hidden' id='HUOMQty_" + indx[1] + "_" + i + "' value='" + val1 + "'></input>", cid)
                ...
                var CancelCell = {
                    ...
                    onClick: function (evt) {
                        ...
                        var val1 = dom.byId("HUOMQty_" + indx[1] + "_" + indx[2]).value

                        dom.byId(pid).textContent = val
                        domConstruct.place("<input  Type='hidden' id='HUOMQty_" + indx[1] + "_" + indx[2] + "' value='" + val1 + "'></input>", "Qty_" + indx[1] + "_" + i)
                        domConstruct.destroy("txt_" + i + "_" + indx[1])
                ...
                var SaveCell = {
                    ...
                    onClick: function (evt) {
                        ...
                        idata = idata + '{"SalesMan":"' + dom.byId("SM_" + i).textContent + '","Product":"' + dom.byId("HPR_" + indx[1]).value + '","Qty":"' + val + '","Org_ID":"' + orgid + '","UOM":"' + uom[0] + '"},'
                ...
                query(".CellBtns_Cancel").on("click", CancelCell.onClick);
                query(".CellBtns_Save").on("click", SaveCell.onClick);
                query(".uom").on("change", UOMQty.onClick);
            });
        ...
        function NumericOnly(e) {
            ...
            if ((parseInt(keycode) >= 48 && parseInt(keycode) <= 57) || parseInt(keycode) == 8 || parseInt(keycode) == 0 || parseInt(keycode) == 46)
                return true;
