...
Partial Public Class DiscountDefinition
        ...
        Try
            If Not IsPostBack Then
                ...
                If Not Request.QueryString("ORGID") Is Nothing Then
                    ...
                    If Not ddl_org.FindItemByValue(Request.QueryString("ORGID")) Is Nothing Then
                        ddl_org.ClearSelection()
                        ddl_org.FindItemByValue(Request.QueryString("ORGID")).Selected = True
                    End If
                End If
                If Not Request.QueryString("Desc") Is Nothing Then
                    lbl_planName.Text = Request.QueryString("Desc")
                    lbl_planName.Visible = True
                    lbl_Plantxt.Visible = True
                    btn_back.Visible = True
                End If
                If Not Request.QueryString("PGID") Is Nothing Then
                    HDiscount_Plan_ID.Value = Request.QueryString("PGID")
                End If
    ...
    Private Sub BindData()
        ...
        Dim x As New DataTable
        x = objProduct.GetDiscountData(Err_No, Err_Desc, IIf(Me.ddl_org.SelectedIndex <= 0, "0", Me.ddl_org.SelectedValue), HDiscount_Plan_ID.Value)
    ...
    Protected Sub btnAddItems_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAddItems.Click
        If ValidationDetails() = False Then
            Try
                ...
                'Check Item exist
                If objProduct.CheckDiscountDataActiveRange(Err_No, Err_Desc, ItemCode, CLng(IIf(Me.txtFromQty.Text = "", "0", Me.txtFromQty.Text)), Me.ddl_org.SelectedValue, Me.StartTime.SelectedDate.Value, Me.EndTime.SelectedDate.Value, CInt(IIf(Me.lblLineID.Text = "", "0", Me.lblLineID.Text)), HDiscount_Plan_ID.Value) = True Then

                    MessageBoxValidation("Discount already define for this item with same slab", "Information")
                    Exit Sub

                End If

                If Me.btnAddItems.Text = "Add" Then
                    success = objProduct.SaveDiscountData(Err_No, Err_Desc, ItemCode, Me.ddl_org.SelectedValue, Me.ddlType.SelectedItem.Value, CLng(IIf(Me.txtFromQty.Text = "", "0", Me.txtFromQty.Text)), CDec(IIf(Me.txtDisValue.Text = "", "0", Me.txtDisValue.Text)), Me.StartTime.SelectedDate.Value, Me.EndTime.SelectedDate.Value, HDiscount_Plan_ID.Value)

                ElseIf Me.btnAddItems.Text = "Update" Then
                    success = objProduct.UpdateDiscountData(Err_No, Err_Desc, Me.lblLineID.Text, ItemCode, Me.ddl_org.SelectedValue, Me.ddlType.SelectedItem.Value, CLng(IIf(Me.txtFromQty.Text = "", "0", Me.txtFromQty.Text)), CDec(IIf(Me.txtDisValue.Text = "", "0", Me.txtDisValue.Text)), Me.StartTime.SelectedDate.Value, Me.EndTime.SelectedDate.Value, HDiscount_Plan_ID.Value)
                End If
    ...
    Private Function ValidationDetails() As Boolean
        ...
        If String.IsNullOrEmpty(ddlItem.SelectedValue) Then
            MessageBoxValidation("Please select a item code/description", "Information")
            sucess = True
            Return sucess
            Exit Function
        End If
        ...
        If Me.txtFromQty.Text = "" Or CDec(IIf(Me.txtFromQty.Text = "", "0", Me.txtFromQty.Text)) = 0 Or CDec(IIf(Me.txtDisValue.Text = "", "0", Me.txtDisValue.Text)) = 0 Or Me.txtDisValue.Text = "" Then
            MessageBoxValidation("Please enter a from quantity and discount value", "Information")
            sucess = True
        ...
        If Me.ddlType.SelectedItem.Text = "PERCENTAGE" Then
            If CDec(IIf(Me.txtDisValue.Text = "", "0", Me.txtDisValue.Text)) > 100 Then
                MessageBoxValidation("Discount Percentage should be less than or equal to 100", "Validation")
                sucess = True
                Return sucess
                Exit Function
            End If
    ...
    Private Sub ddlItem_ItemsRequested(sender As Object, e As RadComboBoxItemsRequestedEventArgs) Handles ddlItem.ItemsRequested
        Dim strgency As String = "0"
        
        Dim Objrep As New SalesWorx.BO.Common.Reports()
        Dim dt As New DataTable
        dt = Objrep.GetItemFromAgency(Err_No, Err_Desc, ddl_org.SelectedValue, strgency, e.Text)
            ...
            If ExcelFileUpload.FileName.ToString.EndsWith(".csv") Or ExcelFileUpload.FileName.ToString.EndsWith(".xls") Or ExcelFileUpload.FileName.ToString.EndsWith(".xlsx") Then
                ...
                Try
                    ...
                    If ViewState("FileType") IsNot Nothing And ViewState("FileName") IsNot Nothing Then
                        ...
                        If TempTbl.Rows.Count > 0 Then
                            ...
                            For idx = 0 To TempTbl.Rows.Count - 1
                                ...
                                If isValidRow = True Then
                                    If objProduct.CheckDiscountDataActiveRange(Err_No, Err_Desc, itemCode, CLng(IIf(FromQty = "", "0", FromQty)), Me.ddl_org.SelectedValue, FromDate, ToDate, "0", HDiscount_Plan_ID.Value) = True Then
                                        RowNo = idx + 2
                        ...
                        If objProduct.UploadDiscount(TempTbl, Me.ddl_org.SelectedValue, Err_No, Err_Desc, HDiscount_Plan_ID.Value) = True Then
                            DeleteExcel()
    ...
    Protected Sub Export_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnExport.Click
        ...
        If Me.ddl_org.SelectedIndex > 0 Then
            ...
            dtOriginal = objProduct.GetDiscountData(Err_No, Err_Desc, OrgID, HDiscount_Plan_ID.Value)
            ...
            For i As Integer = 0 To dtOriginal.Rows.Count - 1
                ...
                drAddItem(3) = DateTime.Parse(dtOriginal.Rows(i)("Valid_From").ToString()).ToString("MM/dd/yyyy")
                drAddItem(4) = DateTime.Parse(dtOriginal.Rows(i)("Valid_To").ToString()).ToString("MM/dd/yyyy")
            ...
            If dtTemp.Rows.Count > 0 Then
                ...
                Dim fn As String = "Discount" + Now.ToString("ddMMMyyHHmmss") + ".xls"
    ...
    Private Sub btn_back_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_back.Click
        Response.Redirect("AdminDiscountPlan.aspx?OID=" & ddl_org.SelectedItem.Value)
    End Sub
    Private Sub btndownloadTemp_Click(sender As Object, e As EventArgs) Handles btndownloadTemp.Click
        Dim Filename As String = System.Configuration.ConfigurationManager.AppSettings("ExcelTemplatePath") & "Item_Discount_Rules.xls"
