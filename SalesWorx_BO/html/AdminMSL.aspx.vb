...
Partial Public Class AdminMSL
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            If Not IsPostBack Then
                ...
                ddlSalesRep.Items.Insert(0, New RadComboBoxItem("--Select a Van/FSR--"))
    ...
    Private Sub ddOraganisation_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddOraganisation.SelectedIndexChanged
        ...
        If ddOraganisation.SelectedItem.Text <> "-- Select a Organization --" Then
            ...
            ddlSalesRep.Items.Add(New RadComboBoxItem("-- Select a van/FSR --"))
            ddlSalesRep.AppendDataBoundItems = True
        ...
        Try
            ...
            If TempTbl IsNot Nothing Then
                ...
                If Me.txtFilter.Text <> "" Then
                    TempTbl.DefaultView.RowFilter = "(Item_Code LIKE '%" & Me.txtFilter.Text & "%')"
                End If
    ...
    Protected Sub btnAdd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAdd.Click
        If Me.ddOraganisation.SelectedIndex > 0 And Me.ddlSalesRep.SelectedIndex > 0 Then
            ...
            End Try
        Else
            MessageBoxValidation("Please select organization/Van.", "Validation")
            Exit Sub
        End If
    End Sub
    ...
    Private Sub btnExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExport.Click
        Try
            ...
            If Me.ddOraganisation.SelectedIndex > 0 Then
                ...
                dtOriginal = objProduct.ExportMSL(Err_No, Err_Desc, ddOraganisation.SelectedItem.Value, ddlSalesRep.SelectedItem.Value)

                Dim dtTemp As New DataTable()

                'Creating Header Row

                dtTemp.Columns.Add("SaleRep_No")
                ...
                For i As Integer = 0 To dtOriginal.Rows.Count - 1
                    ...
                    drAddItem(0) = dtOriginal.Rows(i)("SalesRep_Number").ToString()
                    drAddItem(1) = dtOriginal.Rows(i)("Item_Code").ToString()
                ...
                Try
                    ...
                    If ViewState("FileType") IsNot Nothing And ViewState("FileName") IsNot Nothing Then
                        ...
                        If TempTbl.Columns.Count = 2 Then
                            If Not (TempTbl.Columns(0).ColumnName.ToLower = "salesrep_no" And TempTbl.Columns(1).ColumnName.ToLower = "item_code") Then
                        ...
                        If TempTbl.Rows.Count > 0 Then
                            ...
                            For idx = 0 To TempTbl.Rows.Count - 1
                                Dim SalesRep_NO As String = Nothing
                                ...
                                SalesRep_NO = IIf(TempTbl.Rows(idx)(0) Is DBNull.Value, "0", TempTbl.Rows(idx)(0).ToString())
                                Item = IIf(TempTbl.Rows(idx)(1) Is DBNull.Value, "0", TempTbl.Rows(idx)(1).ToString())
                                
                                Dim dtVan As New DataTable
                                dtVan = objProduct.GetSalesRepID(Err_No, Err_Desc, SalesRep_NO)
                                If dtVan.Rows.Count > 0 Then
                                    VanId = Integer.Parse(dtVan.Rows(0)(0).ToString())
                                    OrgId = dtVan.Rows(0)(1).ToString()
                                End If

                                If VanId = 0 And OrgId = "" Then
                                    RowNo = idx + 2
                                    ErrorText = ErrorText & "Invalid SalesRep No :" & SalesRep_NO & ","
                                    isValidRow = False
                                    TotFailed += 1
                                End If

                                If objProduct.CheckProductValid("tblProduct", Item, OrgId, VanId, Err_No, Err_Desc) = False Then
                                    RowNo = idx + 2
                                    ErrorText = ErrorText & "Invalid Item Code:" & Item & ", van:" & VanId & ","
                                    isValidRow = False
                                    TotFailed += 1
                                End If

                                If objProduct.CheckProductValid("tblMSL", Item, OrgId, VanId, Err_No, Err_Desc) = True Then
                                    RowNo = idx + 2
                                    ErrorText = ErrorText & "Already Exists Item Code:" & Item & ", van:" & VanId & ","
                                    isValidRow = False
                                    TotFailed += 1
                                End If
                                ...
                                Else
                                    If Item <> "0" And OrgId <> "0" Then
                                        objProduct.InsertProduct(OrgId, 0, Item, VanId, Err_No, Err_Desc)
                                        objLogin.SaveUserLog(Err_No, Err_Desc, "X", "PRODUCT MANAGEMENT", "MSL MANAGEMENT", VanId, Item, CType(Session("User_Access"), UserAccess).UserID.ToString(), VanId, OrgId)
                                        st = True
                                    End If
                                End If
                            Next
                    ...
                    Dim fn As String = System.Configuration.ConfigurationManager.AppSettings("ExcelPath") & "UploadLog\" & "MSLLog_" & Now.ToString("yyyyMMdd") + ".txt"
    ...
    Protected Sub ddlSalesRep_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles ddlSalesRep.SelectedIndexChanged
        Try
            If ddlSalesRep.SelectedItem.Text <> "-- Select a van/FSR --" Then
                ''Bind Default List
                BindDefault()
    ...
    Private Sub btnFilter_Click(sender As Object, e As EventArgs) Handles btnFilter.Click
        If Me.ddlSalesRep.SelectedIndex > 0 And Me.ddOraganisation.SelectedIndex > 0 Then
            BindDefault()
            BindSelected()
        Else
            MessageBoxValidation("Please select the organization and van", "Validation")
            Exit Sub
        End If
    End Sub
    ...
    Private Sub btnClearFilter_Click(sender As Object, e As EventArgs) Handles btnClearFilter.Click
        If Me.ddlSalesRep.SelectedIndex > 0 And Me.ddOraganisation.SelectedIndex > 0 Then
            ...
            BindSelected()
        Else
            MessageBoxValidation("Please select the organisation and van", "Validation")
            Exit Sub
        End If
    End Sub
End Class