...
Partial Public Class VanToOrgAndSalesMan
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack() Then
            ...
            FillSourceOrgs()
            BindGrid("1=1")
            If CType(Session.Item("CONTROL_PARAMS"), ControlParams).ENABLE_VAN_DOC_PREFIX = "Y" Then
                gvDivConfig.Columns(5).Visible = True
                Me.txtPrefix.MaxLength = CInt(CType(Session.Item("CONTROL_PARAMS"), ControlParams).VAN_DOC_PREFIX_LENTH)
                divDocPrefix.Visible = True
            Else
                gvDivConfig.Columns(5).Visible = False
                divDocPrefix.Visible = False
                Me.txtPrefix.MaxLength = CInt(CType(Session.Item("CONTROL_PARAMS"), ControlParams).VAN_DOC_PREFIX_LENTH)
            End If
    ...
    Protected Sub btnEdit_Click(ByVal sender As Object, ByVal e As EventArgs)
        Try
            ...
            Dim li As RadComboBoxItem = drpOrganization.FindItemByValue(HidVal.Value)
            drpOrganization.SelectedIndex = drpOrganization.Items.IndexOf(li)
            drpOrganization.Enabled = False

            
            If Not drpOrganization.FindItemByValue(HidVal.Value) Is Nothing Then
                drpOrganization.ClearSelection()
                drpOrganization.FindItemByValue(HidVal.Value).Selected = True
            End If

            Dim Stock_Org_ID As String
            Stock_Org_ID = CType(row.FindControl("HStock_Org_ID"), HiddenField).Value
            If Not drpSourcWH.FindItemByValue(Stock_Org_ID) Is Nothing Then
                drpSourcWH.ClearSelection()
                drpSourcWH.FindItemByValue(Stock_Org_ID).Selected = True
            End If

            Dim Org_HE_ID As String
            Org_HE_ID = CType(row.FindControl("HOrg_HE_ID"), HiddenField).Value
            If Not drpLocalOrg.FindItemByValue(Org_HE_ID) Is Nothing Then
                drpLocalOrg.ClearSelection()
                drpLocalOrg.FindItemByValue(Org_HE_ID).Selected = True
            End If
            drpLocalOrg.Enabled = False

            If Not DrpSalesMan.FindItemByValue(Emp_Code) Is Nothing Then
                DrpSalesMan.ClearSelection()
                DrpSalesMan.FindItemByValue(Emp_Code).Selected = True
            End If

            Dim Prefix As String
            Prefix = CType(row.FindControl("hfPrefix"), HiddenField).Value
    ...
    Protected Sub btnUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            If DrpSalesMan.SelectedValue = "-1" Then
                Me.lblmsgPopUp.Text = "Please select the sales man"
                Me.MPEDetails.VisibleOnPageLoad = True
                Exit Sub
            End If
            ...
            If objOrgConfig.UpdateOrgConfig(Err_No, Err_Desc, Me.txtPrefix.Text) = True Then
                success = True
                MessageBoxValidation("Successfully saved", "Information")
                Me.MPEDetails.VisibleOnPageLoad = False
            Else
                success = False
                MessageBoxValidation("Could not save.", "Information")
                Me.MPEDetails.VisibleOnPageLoad = False
            End If
