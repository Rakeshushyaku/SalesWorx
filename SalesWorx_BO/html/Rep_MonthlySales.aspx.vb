...
Partial Public Class Rep_MonthlySales
    ...
    Private Sub BindData()
        ...
        Try
            ...
            dt = ObjReport.GetMonthlySalesVsReturn(Err_No, Err_Desc, ddlOrganization.SelectedItem.Value, FirstDayOfMonth(txtFromDate.SelectedDate), LastDayOfMonth(txtFromDate.SelectedDate), ddl_Van.SelectedValue, ItemId)

            Dim dtFinal As New DataTable
            dtFinal.Columns.Add("RowID")
            dtFinal.Columns.Add("Item")
            dtFinal.Columns.Add("Day")
            dtFinal.Columns.Add("Sold", Type.GetType("System.Double"))
            dtFinal.Columns.Add("Returned", Type.GetType("System.Double"))
            ...
            If dt.Select("Item <> ''").Count > 0 Then
                ...
                For Each seldr As DataRow In itmTbl.Rows
                    ...
                    dr("Sold") = IIf(seldr("SoldQty") Is DBNull.Value, 0, seldr("SoldQty"))
                    dr("Returned") = IIf(seldr("ReturnedQty") Is DBNull.Value, 0, seldr("ReturnedQty"))
                    dtFinal.Rows.Add(dr)
                Next
    ...
    Private Sub gvRep_CellDataBound(sender As Object, e As PivotGridCellDataBoundEventArgs) Handles gvRep.CellDataBound
        Try
            If TypeOf e.Cell Is PivotGridColumnHeaderCell Then
                If e.Cell.Text.IndexOf("Total Sum of Sold") >= 0 Then
                    e.Cell.Text = "Total Sold"
                End If
                If e.Cell.Text.IndexOf("Total Sum of Returned") >= 0 Then
            ...
            If TypeOf e.Cell Is PivotGridDataCell Then
                ...
                If cell.CellType = PivotGridDataCellType.DataCell OrElse cell.CellType = PivotGridDataCellType.RowTotalDataCell Then
                    ...
                    If TryCast(cell.Field, PivotGridAggregateField).DataField = "Returned" Or TryCast(cell.Field, PivotGridAggregateField).DataField = "Sold" Then
    ...
    Sub Export(format As String)
        ...
        Try
            ...
            If format = "PDF" Then
                ...
                Response.AddHeader("Content-disposition", "attachment;filename=MonthlySoldvsRetuned.pdf")
                Response.AddHeader("Content-Length", bytes.Length)
            ElseIf format = "Excel" Then
                Response.ContentType = "application/excel"
                Response.AddHeader("Content-disposition", "filename=MonthlySoldvsRetuned.xls")
                Response.AddHeader("Content-Length", bytes.Length)
            End If
