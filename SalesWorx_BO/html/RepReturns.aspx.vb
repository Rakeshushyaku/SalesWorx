...
Partial Public Class RepReturns
    ...
    Private ReportPathCreditNote As String = "CreditDetailsNew"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            If CType(Session.Item("CONTROL_PARAMS"), ControlParams).ENABLE_PRINT_RETURN_BO = "Y" Then
                ENABLE_PRINT_RETURN_BO = True
                gvRep.MasterTableView.GetColumn("CreditNote").Display = True
            Else
                ENABLE_PRINT_RETURN_BO = False
                gvRep.MasterTableView.GetColumn("CreditNote").Display = False
    ...
    Private Sub BtnPDF_ClickCreditNotes(sender As Object, e As EventArgs) Handles BtnExportPDFCreditNote.Click
        Try

            ExportCreditNote("PDF")
    ...
    Sub ExportCreditNote(format As String)

        
        Dim rview As New Microsoft.Reporting.WebForms.ReportViewer()
        rview.ServerReport.ReportServerCredentials = New CustomReportServerCredentials()
        rview.ServerReport.ReportServerUrl = New System.Uri(AppSettings("ReportServer"))
        rview.ServerReport.ReportPath = String.Format("{0}{1}", AppSettings("ReportPath"), Me.ReportPathCreditNote)



        Dim RefNo As New ReportParameter
        RefNo = New ReportParameter("RefNo", HiddenFieldRow1.Value)
       
        rview.ServerReport.SetParameters(New ReportParameter() {RefNo})
        ...
        If format = "PDF" Then
            ...
            Response.AddHeader("Content-disposition", "attachment;filename=CreditNotes.pdf")
            Response.AddHeader("Content-Length", bytes.Length)
        ElseIf format = "Excel" Then
            Response.ContentType = "application/excel"
            Response.AddHeader("Content-disposition", "filename=CreditNotes.xls")
            Response.AddHeader("Content-Length", bytes.Length)
        End If
    ...
    Private Sub BindReport()
        Try
            ...
            Dim dt As New DataTable
            dt = ObjReport.GetReturns(Err_No, Err_Desc, SearchQuery, ddlOrganization.SelectedItem.Value, CDate(txtFromDate.SelectedDate).ToString("dd-MMM-yyyy"), CDate(txtToDate.SelectedDate).ToString("dd-MMM-yyyy"))
            gvRep.DataSource = dt
            gvRep.DataBind()
            ...
            If ShowDrillDownforDelNote.ToUpper = "Y" Then
                gvRep.MasterTableView.Columns.FindByUniqueName("LinkedOrder").Visible = True
            Else
                gvRep.MasterTableView.Columns.FindByUniqueName("LinkedOrder").Visible = False
            End If
