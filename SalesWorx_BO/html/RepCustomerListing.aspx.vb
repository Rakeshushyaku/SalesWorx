...
Partial Public Class RepCustomerListing
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            End Try
        Else
            VanList.VisibleOnPageLoad = False
        End If
    End Sub
    ...
    Private Sub BindReport()
        Try
            ...
            Dim query = (From Cust In dt _
                        Group Cust By key = Cust.Field(Of String)("CustStatus") Into Group _
                        Select CustType = key, Count = Group.Count()).ToList
            ...
            For Each x In query
                ...
                StrSummary = StrSummary & "<div class='col-sm-6 col-md-3'><div class='widgetblk'>" & x.CustType & " Customers<div class='text-primary'>" & Format(x.Count, "#,##0") & "</div></div></div>"

                i = i + 1
            Next

            Dim query1 = (From Cust In dt _
                        Group Cust By key = Cust.Field(Of String)("CustType") Into Group _
                        Select CustType = key, Count = Group.Count()).ToList
            ...
            If ClientCode = "AHG" Then
                gvRep.MasterTableView.Columns.FindByUniqueName("Payment_Term").Visible = True

            Else
                gvRep.MasterTableView.Columns.FindByUniqueName("Payment_Term").Visible = False
            End If
    ...
    Protected Sub gvRep_ItemCommand(sender As Object, e As GridCommandEventArgs) Handles gvRep.ItemCommand
        Try
            If e.CommandName = "Van" Then
                Dim dataItem As GridDataItem = CType(e.Item, GridDataItem)
                Dim item As GridDataItem = DirectCast(e.Item, GridDataItem)
                Dim lbl_SiteUseID As Label = DirectCast(item.FindControl("lblSite_Use_ID"), Label)
                Dim lbl_CustomerID As Label = DirectCast(item.FindControl("lblCustomer_ID"), Label)
                Dim ObjReport As New SalesWorx.BO.Common.Reports
                Dim dt_van As New DataTable
                dt_van = ObjReport.GetCustomerVanListing(Err_No, Err_Desc, lbl_SiteUseID.Text, lbl_CustomerID.Text)
                Me.gvVan.DataSource = dt_van
                Me.gvVan.DataBind()
                Me.VanList.VisibleOnPageLoad = True
