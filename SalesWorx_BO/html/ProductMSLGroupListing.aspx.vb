...
Partial Public Class ProductMSLGroupListing
    Inherits System.Web.UI.Page
    Private dtErrors As New DataTable
    Dim Err_No As Long
    Dim Err_Desc As String
    Private Const ModuleName As String = "ProductMSLGroupListing.aspx"
    ...
    Private Sub BindData()
        ...
        Try
            If dtSearch.Rows.Count > 0 Then
                dtSearch.Rows.Clear()
            End If
            dtSearch = obj.GetProductMSLGroup(Err_No, Err_Desc, 0)
            ...
            Me.dgv.DataBind()
            Session.Remove("MSLGroupID")
            Session("MSLGroupID") = dtSearch
    ...
    Protected Sub btnAdd_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAdd.Click
        ...
        Response.Redirect("AdminProductMSLGroup.aspx?Desc=New&PGID=0", False)
    End Sub
    ...
    Protected Sub dgv_ItemCommand(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles dgv.ItemCommand
        Try
            If (e.CommandName = "View") Then
                ...
                Response.Redirect("AdminProductMSLGroup.aspx?Desc=" & GroupName & "&PGID=" & PGID.Text & "&ORGID=" & ORGID.Text, False)
            End If
            If (e.CommandName = "DeleteGroup") Then
                Dim row As GridDataItem = DirectCast(DirectCast(e.CommandSource, Control).NamingContainer, GridDataItem)
                Dim PGID As Label = DirectCast(row.FindControl("lblPGID"), Label)
                Dim obj As New Product
                If obj.DeleteProductMSLGroupALL(Err_No, Err_Desc, PGID.Text) = True Then
                    BindData()
                End If
            End If
