...
Partial Public Class ImportStock
    ...
    Private Const ModuleName As String = "ImportStock.aspx"
    ...
    Function ImportFile(ByVal File As String, ByRef RetMessage As String) As Boolean
        Try
            ...
            If bRetVal = True Then
                ...
                FinalTbl.Columns.Add("Lot_Qty_In_Stock_UOM")
                ...
                If TempTbl.Rows.Count > 0 Then
                    ...
                    For Each dr In TempTbl.Rows
                        ...
                        Lot_number = dr(2).ToString



                        Dim Ex_date As String
                        Ex_date = dr(3).ToString
                        ...
                        If Van = "" Then
                            ...
                            ErrorDr(3) = dr(3)
                            ErrorDr(4) = dr(4)
                            ErrorDr(5) = " Van code Can't be empty"
                            ErrorDr(6) = Rowno
                        ...
                        ElseIf Item = "" Then
                            ...
                            ErrorDr(3) = dr(3)
                            ErrorDr(4) = dr(4)
                            ErrorDr(5) = "Item code Can't be empty"
                            ErrorDr(6) = Rowno
                            ErrorTbl.Rows.Add(ErrorDr)
                            bImported = False
                        ElseIf IsValidItem(ddl_Organization.SelectedItem.Value, Item) = False Then
                            ...
                            ErrorDr(3) = dr(3)
                            ErrorDr(4) = dr(4)
                            ErrorDr(5) = "Inactive/Invalid product"
                            ErrorDr(6) = Rowno
                            ErrorTbl.Rows.Add(ErrorDr)
                            bImported = False
                        ...
                        ElseIf HasLots(Item) And Lot_number = "" Then
                            ...
                            ErrorDr(3) = dr(3)
                            ErrorDr(4) = dr(4)
                            ErrorDr(5) = "Lot Number Cannot be empty "
                            ErrorDr(6) = Rowno
                            ErrorTbl.Rows.Add(ErrorDr)
                            bImported = False
                        ...
                        ElseIf HasLots(Item) And Lot_number.Length > 50 Then
                            ...
                            ErrorDr(3) = dr(3)
                            ErrorDr(4) = dr(4)
                            ErrorDr(5) = "Lot number beyond the length"
                            ErrorDr(6) = Rowno
                            ErrorTbl.Rows.Add(ErrorDr)
                            bImported = False
                        ElseIf Not ValidItem(Item, Item_ID) Then
                            ...
                            ErrorDr(3) = dr(3)
                            ErrorDr(4) = dr(4)
                            ErrorDr(5) = "Item does not exist"
                            ErrorDr(6) = Rowno
                            ErrorTbl.Rows.Add(ErrorDr)
                            bImported = False

                        ElseIf Not ValidateQty(Val(qty).ToString(), Item, ddl_Organization.SelectedItem.Value) Then  ' ElseIf Not ValidQty((Val(qty) * Val(Conversion)).ToString) Then
                            Dim ErrorDr As DataRow
                            ErrorDr = ErrorTbl.NewRow
                            ErrorDr(0) = dr(0)
                            ErrorDr(1) = dr(1)
                            ...
                            ErrorDr(5) = "Qty is not valid:(" & (Val(qty) * Val(Conversion)).ToString & ")"
                            ErrorDr(6) = Rowno
                            ErrorTbl.Rows.Add(ErrorDr)
                            bImported = False
                        Else
                            ...
                            finalDr(8) = ObjStock.GetItemUOM(Err_No, Err_Desc, ddl_Organization.SelectedItem.Value, dr(1).ToString)
                    ...
                    If FinalTbl.Rows.Count > 0 Then
                        ...
                        ds_st_unconfirm = ObjStock.Get_ImportStockUnConfirm(Err_No, Err_Desc, xml_s, ddl_Organization.SelectedItem.Value)


                        Session.Remove("Stock_Unconfirm")
                        Session("Stock_Unconfirm") = ds_st_unconfirm

                        If ds_st_unconfirm.Tables.Count > 0 Then
    ...
    Protected Sub btnView_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Try
            ...
            Dim distinctValues As DataTable = view.ToTable(True, "item_code", "Lot_Number", "ExpiryDate")
            ...
            log.Error("7")
            Dim totals1 = (From [rw] As DataRow In dt_ Group [rw] By item_code = rw("item_code"), Expiry_Date = rw("ExpiryDate") Into g = Group Select New With {Key item_code, Expiry_Date, .Total = g.Sum(Function(x) x("Lot_QtyS")).ToString()})
            ...
            For Each r As DataRow In distinctValues.Rows
                ...
                If HasLots(r(0).ToString()) Then
                    ...
                    Try
                        ...
                        Dim sum As Object
                        sum = dt_.Compute("Sum(Lot_QtyS)", "item_code = '" & r(0).ToString() & "' AND Lot_Number='" & r(1).ToString() & "' AND ExpiryDate='" & r(2).ToString() & "' ")

                        Dim dth As New DataTable
                        dth = dt_.Select("item_code= '" & r(0).ToString() & "' AND Lot_Number='" & r(1).ToString() & "' AND ExpiryDate='" & r(2).ToString() & "'").CopyToDataTable()
                ...
                Else
                    ...
                    Try
                        ...
                        Try
                            Dim drow As DataRow = dt_v.Select("item_code='" & r(0).ToString() & "'")(0)
                        ...
                        sumObject = dt_.Compute("Sum(Lot_QtyS)", "item_code = '" & r(0).ToString() & "'")
                        Dim dt_1 As New DataTable
                        dt_1 = dt_.Select("item_code= '" + r(0).ToString() + "'").CopyToDataTable()
                        Dim Row_itm As DataRow
                        Row_itm = dt_v.NewRow()
                        Row_itm.Item(0) = HRow_vancode
                        Row_itm.Item(1) = r(0).ToString()
    ...
    Private Function DoXLSUpload(ByVal filename As String, ByRef bfileformat As Boolean) As DataTable
        ...
        Try
            ...
            Try
                ...
                oleda.Fill(dtImport)
                If dtImport.Columns.Count = 5 Then
                    If dtImport.Columns(0).ColumnName.ToUpper = "VAN_CODE" And dtImport.Columns(1).ColumnName.ToUpper = "ITEM_CODE" And dtImport.Columns(2).ColumnName.ToUpper = "LOT_NUMBER" And dtImport.Columns(3).ColumnName.ToUpper = "EXPIRY_DATE" And dtImport.Columns(4).ColumnName.ToUpper = "LOT_QTY_IN_STOCK_UOM" Then
                        bfileformat = True
                    End If
                End If
            Catch ex As Exception
            ...
            If Ds_stock.Tables.Count = 3 Then
                ...
                For Each dr As DataRow In dt_s.Rows
                    ...
                    If dt_pr.Rows.Count > 0 Then
                        ...
                        dr_I("Qty") = dr("Lot_Qty_In_Stock_UOM")
                ...
                For Each dr As DataRow In dt_s.Rows
                    ...
                    dr_L("Lot_Qty") = dr("Lot_Qty_In_Stock_UOM")
                    dr_L("Lot_Type") = "U"
    ...
    Protected Sub btndownloadTemplate_Click(sender As Object, e As EventArgs) Handles btndownloadTemplate.Click
        ...
        Dim Filename As String = System.Configuration.ConfigurationManager.AppSettings("ExcelTemplatePath") & "Stock_Template.xls"
    ...
    Function ValidateQty(ByVal Qty As String, ByVal item_code As String, ByVal Org_ID As String)
        Dim uom As String = ""

        Dim Con_ As String = "1"
        Dim bRetVal As Boolean
        Try
            log.Error(Qty & "," & item_code & "," & Org_ID)
            uom = ObjStock.GetUOMforConverion(Err_No, Err_Desc, Org_ID, item_code)
            log.Error(uom & " - uom")
            Con_ = ObjStock.GetConversion(Err_No, Err_Desc, Org_ID, item_code, uom)
            log.Error(Con_ & " - Con")
            Dim num As Integer
            If Integer.TryParse((Val(Qty) * Val(Con_)), num) Then
                bRetVal = True
            Else
                bRetVal = False
            End If
