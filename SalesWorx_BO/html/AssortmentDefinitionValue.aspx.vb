...
Partial Public Class AssortmentDefinitionValue
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If (Not IsPostBack) Then
            ...
            Me.lblTransType.Text = Request.QueryString("TRANTYPE").ToString()
        ...
        Else
            ...
            If MPInfo.VisibleOnPageLoad Then
    ...
    Private Sub ddlOrdCode_ItemsRequested(sender As Object, e As RadComboBoxItemsRequestedEventArgs) Handles ddlOrdCode.ItemsRequested
        ...
        If MPInfo.VisibleOnPageLoad Then
            MPInfo.VisibleOnPageLoad = False
        End If
        Me.ddlOrdUOM.SelectedIndex = 0
        Me.ddlOrdUOM.Items.Clear()
        Me.ddlOrdUOM.Text = ""
    ...
    Private Sub ddlgetDesc_ItemsRequested(sender As Object, e As RadComboBoxItemsRequestedEventArgs) Handles ddlgetDesc.ItemsRequested
        Me.ddlGetUOM.SelectedIndex = 0
        Me.ddlGetUOM.Items.Clear()
        Me.ddlGetUOM.Text = ""
    ...
    Protected Sub btnOrdAdd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnOrdAdd.Click
        ...
        If String.IsNullOrEmpty(ddlOrdUOM.SelectedValue) Or ddlOrdUOM.SelectedIndex <= 0 Then
            MessageBoxValidation("Please select order item UOM", "Information")
            Exit Sub
        End If
        dtOrdItems = Session("dtOrdItems")
    ...
    Protected Sub btnGetAdd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGetAdd.Click
        If String.IsNullOrEmpty(ddlGetUOM.SelectedValue) Or ddlGetUOM.SelectedIndex <= 0 Then
            MessageBoxValidation("Please select bonus item UOM", "Information")
            Exit Sub
        End If
        ...
        If st = True Then
            ...
            N("Description") = (New SalesWorx.BO.Common.Product).GetItemNameFromCode(Err_No, Err_Desc, s(0), lblOrgID.Text)
            'N("UOM") = UOM
            N("UOM") = Me.ddlGetUOM.SelectedValue
            N("GetItem") = "Y"
            N("IsMandatory") = "False"
    ...
    Protected Sub btnAddSlab_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddSlab.Click
        ...
        If Me.txtFromQty.Text = "" Or CDec(IIf(Me.txtFromQty.Text = "", "0", Me.txtFromQty.Text)) = 0 Or Me.txtFromQty.Text = "" Or CDec(IIf(Me.txtToQty.Text = "", "0", Me.txtToQty.Text)) = 0 Or Me.txtGetQty.Text = "" Or CDec(IIf(Me.txtGetQty.Text = "", "0", Me.txtGetQty.Text)) = 0 Then
            MessageBoxValidation("Please enter a valid from,to and get Value", "Validation")
            Exit Sub
        End If
        ...
        If Me.txtFromQty.Text <> "" And Me.txtFromQty.Text <> "0" And Me.txtToQty.Text <> "" And Me.txtToQty.Text <> "0" Then
            If CDec(Me.txtToQty.Text) < CDec(Me.txtFromQty.Text) Then
                MessageBoxValidation("To Value should be greater than from Value", "Validation")
                Exit Sub
            End If
        End If
        ...
        If x.Rows.Count > 0 Then
            For Each dr As DataRow In x.Rows
                If dr("GetItem").ToString() = "N" Then
                    ...
                    oRow("Description") = dr("ItemCode").ToString() & "-" & dr("Description").ToString()
    ...
    Sub LoadGetUOM()
        Try
            If Not String.IsNullOrEmpty(ddlGetCode.SelectedValue) Then
                ...
                ids = ddlGetCode.SelectedValue.Split("$")
                Me.ddlGetUOM.Items.Clear()
                Me.ddlGetUOM.DataSource = objProduct.GetProductUOM_ItemCode(Err_No, Err_Desc, Me.lblOrgID.Text, ids(0))
                ddlGetUOM.Items.Add(New RadComboBoxItem("Select UOM"))
                ddlGetUOM.AppendDataBoundItems = True
                ddlGetUOM.DataValueField = "UOM"
                ddlGetUOM.DataTextField = "UOM"
                ddlGetUOM.DataBind()
            End If
    ...
    Sub LoadOrdUOM()
        Try
            If Not String.IsNullOrEmpty(ddlOrdCode.SelectedValue) Then
                ...
                ids = ddlOrdCode.SelectedValue.Split("$")
                Me.ddlOrdUOM.Items.Clear()
                Me.ddlOrdUOM.DataSource = objProduct.GetProductUOM_ItemCode(Err_No, Err_Desc, Me.lblOrgID.Text, ids(0))
                ddlOrdUOM.Items.Add(New RadComboBoxItem("Select UOM"))
                ddlOrdUOM.AppendDataBoundItems = True
                ddlOrdUOM.DataValueField = "UOM"
                ddlOrdUOM.DataTextField = "UOM"
                ddlOrdUOM.DataBind()
            End If
