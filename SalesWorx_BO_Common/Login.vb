...
Public Class Login
    ...
    Public Function LoadTreeRights(ByRef Error_No As Long, ByRef Error_Desc As String) As DataTable
        Dim dtUser As DataTable
        dtUser = objDALLogin.LoadRights(Error_No, Error_Desc)
        Return dtUser
    End Function
    Public Property ControlParams() As ControlParams
        Get
            ControlParams = _objParams
        End Get
        Set(ByVal Value As ControlParams)
            _objParams = Value
        End Set
    End Property

    Public Function ValidateUser(ByRef Error_No As Long, ByRef Error_Desc As String, ByVal mode As String, ByRef dtUser As DataTable) As Boolean
                ...
                If dtUser.Rows.Count > 0 Then
                    ...
                    If dtParams.Rows.Count > 0 Then
                        ...
                        For Each dr As DataRow In dtParams.Rows
                            ...
                            If dr("Control_key").ToString() = "TARGET_VALUE_TYPE" Then
                                _objParams.TARGET_VALUE_TYPE = dr("Control_Value").ToString()
                            End If
                            If dr("Control_key").ToString() = "MSG_MODULE_VERSION" Then
                                _objParams.MSG_MODULE_VERSION = dr("Control_Value").ToString()
                            End If
                            If dr("Control_key").ToString() = "SHOW_GROUPS_IN_BO_REPORTS" Then
                                _objParams.SHOW_GROUPS_IN_BO_REPORTS = dr("Control_Value").ToString()
                            End If
                            If dr("Control_key").ToString() = "USE_DISTR_IN_CALLS" Then
                                _objParams.USE_DISTR_IN_CALLS = dr("Control_Value").ToString()
                            End If
                            If dr("Control_key").ToString() = "FSR_TARGET_TYPE" Then
                                _objParams.FSR_TARGET_TYPE = dr("Control_Value").ToString()
                            End If

                            If dr("Control_key").ToString() = "FSR_TARGET_PRIMARY" Then
                                _objParams.FSR_TARGET_PRIMARY = dr("Control_Value").ToString()
                            End If
                            If dr("Control_key").ToString() = "CLIENT_CODE" Then
                                _objParams.CLIENT_CODE = dr("Control_Value").ToString()
                            End If
                            If dr("Control_key").ToString() = "ENABLE_VAN_DOC_PREFIX" Then
                                _objParams.ENABLE_VAN_DOC_PREFIX = dr("Control_Value").ToString()
                            End If
                            If dr("Control_key").ToString() = "VAN_DOC_PREFIX_LENTH" Then
                                _objParams.VAN_DOC_PREFIX_LENTH = dr("Control_Value").ToString()
                            End If
                            If dr("Control_key").ToString() = "ENABLE_PRINT_RETURN_BO" Then
                                _objParams.ENABLE_PRINT_RETURN_BO = dr("Control_Value").ToString()
                            End If
                        Next
                    Else
                        ...
                        _objParams.TARGET_VALUE_TYPE = "Q"
                        _objParams.MSG_MODULE_VERSION = "1"
                        _objParams.FSR_TARGET_TYPE = "N"
                        _objParams.FSR_TARGET_PRIMARY = "Customer No"
                        _objParams.CLIENT_CODE = "N"
                        _objParams.ENABLE_VAN_DOC_PREFIX = "N"
                        _objParams.VAN_DOC_PREFIX_LENTH = "5"
                        _objParams.ENABLE_PRINT_RETURN_BO = "N"
                    End If
    ...
    Public Function GetMenu(ByRef MainMenu As String, ByRef SubMenu As String) As Boolean
        ...
        GetMenu = sRetMenu
    End Function
    Function GetuserGroups(ByRef Err_No As Long, ByRef Err_Desc As String, ByVal SearchParams As String) As DataTable
        Try
            Return objDALLogin.GetuserGroups(Err_No, Err_Desc, SearchParams)
        Catch ex As Exception
            Throw ex
        End Try
    End Function
    Public Function GetUserGroupDetails(ByRef Err_No As Long, ByRef Err_Desc As String, ByVal ID As String) As DataTable
        Try
            Return objDALLogin.GetUserGroupDetails(Err_No, Err_Desc, ID)
        Catch ex As Exception
            Throw ex
        End Try
    End Function
    Public Function IfGroupNameExists(ByRef Err_No As Long, ByRef Err_Desc As String, ByVal ID As String, Name As String) As Boolean
        Try
            Return objDALLogin.IfGroupNameExists(Err_No, Err_Desc, ID, Name)
        Catch ex As Exception
            Throw ex
        End Try
    End Function
    Function DeleteGroup(ByRef Err_No As Long, ByRef Err_Desc As String, ByVal ID As String) As Boolean
        Return objDALLogin.DeleteGroup(Err_No, Err_Desc, ID)
    End Function
    Function SaveGroup(ByRef Err_No As Long, ByRef Err_Desc As String, name As String, SalesRepIDs As String, ByVal ID As String, UserID As String, Org_HE_ID As String) As Boolean
        Return objDALLogin.SaveGroup(Err_No, Err_Desc, name, SalesRepIDs, ID, UserID, Org_HE_ID)
    End Function
